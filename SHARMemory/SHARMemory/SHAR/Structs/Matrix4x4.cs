...
public struct Matrix4x4 : IEquatable<Matrix4x4>
{
    ...
    public readonly void Transform(Vector3 src, ref Vector3 dest)
    {
    ...
    public readonly bool SameMatrix(Matrix4x4 matrix, float epsilon = 0.000001f) => SameTranslation(matrix, epsilon) && SameRotation(matrix, epsilon);

    public readonly bool SameTranslation(Matrix4x4 matrix, float epsilon = 0.000001f)
    {
    ...
    public readonly bool SameRotation(Matrix4x4 matrix, float epsilon = 0.000001f)
    {
    ...
    public readonly Vector3 GetPosition() => new(M41, M42, M43);

    public override readonly string ToString() => $"{{ {{M11:{M11} M12:{M12} M13:{M13} M14:{M14}}} {{M21:{M21} M22:{M22} M23:{M23} M24:{M24}}} {{M31:{M31} M32:{M32} M33:{M33} M34:{M34}}} {{M41:{M41} M42:{M42} M43:{M43} M44:{M44}}} }}";

    public override readonly bool Equals(object obj) => obj is Matrix4x4 x && Equals(x);
